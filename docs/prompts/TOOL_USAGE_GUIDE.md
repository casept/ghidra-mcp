...
| `GHIDRA_MCP_ALLOW_SCRIPTS` | Set to `1`, `true`, or `yes` to enable `/run_script_inline` and `/run_ghidra_script`. **Off by default as of v5.4.1** (breaking change — these endpoints execute arbitrary Java against the Ghidra process). GUI alternative: **Edit > Tool Options > GhidraMCP HTTP Server > Allow Script Execution** (env var OR checkbox enables scripts). |
